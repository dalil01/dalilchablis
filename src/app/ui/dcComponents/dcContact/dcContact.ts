...
export class dcContact extends dcComponent {
	...
	public buildUI(): void {
		const socialNetworksContainer = _UDom.div({ className: CONTACT_CSS_CLASSNAMES.SOCIAL_NETWORKS_CONTAINER });
		const socialNetworksTitle = _UDom.h2({
			innerText: dcTranslator.T(dcTranslation.SOCIAL_NETWORKS),
			className: CONTACT_CSS_CLASSNAMES.SOCIAL_NETWORKS_TITLE
		});
		const socialNetworks = _UDom.div({ className: CONTACT_CSS_CLASSNAMES.SOCIAL_NETWORKS });
		...
		const mailFormContainer = _UDom.div({ className: CONTACT_CSS_CLASSNAMES.FORM_CONTAINER });
		const mailTitle = _UDom.h2({
			innerText: dcTranslator.T(dcTranslation.EMAIL),
			className: CONTACT_CSS_CLASSNAMES.FORM_TITLE
		});
		mailFormContainer.appendChild(mailTitle);
		
		const mailFormInputs: dcFormInputType[] = [
			{
				icon: DcIcons.DcIconUser,
